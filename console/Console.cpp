...
if (cursor_x < 0 || cursor_x >= terminal_w ||
    cursor_y < 0 || cursor_y >= terminal_h)
{
    return;
}
...
void Console::draw_xy_char(int x, int y){
        ...
        unsigned char ch = (unsigned char)main_text[y][x].ch;
//	const unsigned char *font_ch=  ProFont6x11 + 5 + 12*main_text[y][x].ch + 1;
        const unsigned char *font_ch = ProFont6x11 + 5 + 12 * ch + 1;
...
void Console::draw_cur_char(){
        unsigned char ch = (unsigned char)main_text[cursor_y][cursor_x].ch;
...
void Console::save_to_file(VMWSTR path)
{
    VMFILE f = vm_file_open(
        path,
    ...
    //
    // Find first and last non-empty history lines
    //
    int first_history = -1;
    int last_history = -1;
